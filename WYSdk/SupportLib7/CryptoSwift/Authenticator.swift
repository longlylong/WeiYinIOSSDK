...
public enum Authenticator {
    ...
    */
    ...
    case HMACAuth(key: Array<UInt8>, variant:HMAC.Variant)
    ...
    */
    public func authenticate(message: Array<UInt8>) throws -> Array<UInt8> {
        ...
        case .Poly1305(let key):
            guard let auth = try? Poly1305(key: key).authenticate(message) else {
                throw Error.AuthenticateError
            }
            return auth
        case .HMACAuth(let key, let variant):
            guard let auth =  HMAC(key: key, variant: variant)?.authenticate(message) else {
                throw Error.AuthenticateError
            }
            return auth
        }
